# Claude Code Project Instructions — seccompiler-kotlin

## Project Overview

This is **seccompiler-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`seccompiler`](https://crates.io/crates/seccompiler).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/seccompiler/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/seccompiler>
package io.github.kotlinmania.seccompiler
```
