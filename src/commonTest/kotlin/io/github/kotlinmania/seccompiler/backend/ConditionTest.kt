...
internal class ConditionTest {
    ...
    fun testGetDataOffsets() {
        ...
        // Upstream additionally constructs the Linux seccomp data layout and indexes into it via
        // raw pointer arithmetic to confirm the offset values land in the right 32-bit unsigned
        // slots of the packed layout. KMP has no libc, so the equivalent check synthesises the
        // same packed layout into a 64-byte buffer and reads the two 32-bit words back at the
        // computed offsets.
        val data = ByteArray(64)
        // args[1] = UInt.MAX_VALUE.toULong() + 1uL = 0x1_0000_0000 (little-endian).
        ...
        // unsigned 32-bit chunks being read), and args[1] is the only argument the test inspects.
